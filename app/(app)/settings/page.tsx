...
function SettingsContent() {
  ...
  const toggleWarmup = useMutation({
    mutationFn: async ({ id, warmupStatus }: { id: number; warmupStatus: string }) => {
      const res = await fetch("/api/email-accounts", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id, warmupStatus }),
      });
      if (!res.ok) throw new Error("Failed to update warmup settings");
      return res.json();
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["email-accounts"] }),
  });
  ...
  return (
    <div className="p-6 max-w-2xl flex flex-col gap-8">
      ...
      {/* Account / Profile Management Info */}
      <section className="bg-white rounded-xl border border-border p-6 flex flex-col gap-6">
        <div>
          <h2 className="text-base font-semibold text-text">Profile Settings</h2>
          <p className="text-xs text-muted mt-1">Manage your account credentials, email address, and plan settings.</p>
        </div>
        ...
        >
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                className="h-10 px-3 text-sm bg-white border border-border rounded-md focus:outline-none focus:ring-2 focus:ring-primary/40 font-medium text-text"
              />
            </div>
            ...
            <div className="flex flex-col gap-1">
              ...
              <select
                ...
                className="h-10 px-3 text-sm bg-white border border-border rounded-md focus:outline-none focus:ring-2 focus:ring-primary/40 font-medium text-text cursor-pointer"
              >
          ...
          <div className="md:col-span-2 flex justify-end">
            <Button type="submit" loading={updateProfile.isPending} className="px-6">
              Save Profile Edits
            </Button>
          </div>
        </form>
      ...
      <section>
        ...
        <div className="flex flex-col gap-3">
          {accounts?.map(account => {
            ...
            const effectiveLimit = isWarmupActive 
              ? Math.min(account.dailyLimit, 5 + (account.warmupDay - 1) * 5)
              : account.dailyLimit;
            ...
            return (
              <div key={account.id} className="bg-white rounded-xl border border-border p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full bg-red-100 flex items-center justify-center text-red-600 font-semibold text-sm shrink-0">
                    ...
                    <div className="mt-1 flex flex-col gap-0.5">
                      <div className="flex items-center gap-2">
                        <div className="h-1.5 w-24 bg-background rounded-full overflow-hidden">
                          <div className="h-full rounded-full bg-primary" style={{ width: `${Math.min((account.dailySent / effectiveLimit) * 100, 100)}%` }} />
                        </div>
                        <span className="text-xs text-muted">
                          {account.dailySent}/{effectiveLimit} sent today {isWarmupActive && `(max ${account.dailyLimit})`}
                        </span>
                      </div>
                      {isWarmupActive && (
                        <p className="text-[10px] font-bold text-amber-600">
                          🔥 Warm-up Day {account.warmupDay}
                        </p>
                      )}
                      {isWarmupCompleted && (
                ...
                <div className="flex items-center gap-2 self-end sm:self-center">
                  <button
                    onClick={() => {
                      const nextStatus = isWarmupActive ? "inactive" : "active";
                      toggleWarmup.mutate({ id: account.id, warmupStatus: nextStatus });
                    }}
                    className={cn(
                      "px-2.5 py-1.5 rounded border text-xs font-semibold cursor-pointer transition-colors duration-150",
                      isWarmupActive 
                        ? "bg-amber-50 text-amber-700 border-amber-200 hover:bg-amber-100" 
                        : isWarmupCompleted
                        ? "bg-emerald-50 text-emerald-700 border-emerald-200 cursor-not-allowed opacity-80"
                        : "bg-background text-muted border-border hover:bg-background/80"
                    )}
                    disabled={isWarmupCompleted}
                  >
                    {isWarmupActive ? "Pause Warm-up" : isWarmupCompleted ? "Warm-up Done" : "Start Warm-up"}
                  </button>
                  <button onClick={() => disconnect.mutate(account.id)} className="text-muted hover:text-red-500 cursor-pointer transition-colors p-1 border border-transparent rounded hover:border-border">
      ...
      {/* Team Seats / Agency Section */}
      {user?.plan === "agency" && (
        <section className="bg-white rounded-xl border border-border p-6 flex flex-col gap-6">
          <div>
            <h2 className="text-base font-semibold text-text">Team Collaboration</h2>
            <p className="text-xs text-muted mt-1">Manage team members who can access and collaborate on this agency account.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <form
              ...
              className="flex flex-col gap-3 p-4 border border-border rounded-xl md:col-span-1 bg-background/20"
            >
              ...
              <div className="flex flex-col gap-1">
                ...
                <input
                  ...
                  className="h-9 px-3 text-xs bg-white border border-border rounded-md focus:outline-none focus:ring-2 focus:ring-primary/40 font-medium text-text"
              ...
              <Button type="submit" disabled={inviteMember.isPending} className="w-full h-9 text-xs font-semibold">
                {inviteMember.isPending ? "Inviting..." : "Add Member"}
              </Button>
            </form>
              ...
              <div className="flex flex-col gap-2.5 max-h-60 overflow-y-auto pr-1">
                {teamMembers?.map((m: any) => (
                  <div key={m.id} className="bg-white rounded-lg border border-border p-3 flex justify-between items-center text-xs">
                    <div>
                      <p className="font-semibold text-text">{m.email}</p>
                      <p className="text-[10px] text-muted capitalize font-medium">{m.role}</p>
                    </div>
                    {m.role !== "owner" && (
                      <button
                        type="button"
                        ...
                        className="text-muted hover:text-red-500 cursor-pointer p-1 rounded hover:bg-red-50/50"
                      >
